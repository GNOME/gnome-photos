...
struct _PhotosGeglBufferLoader
{
  ...
  gboolean closed;
  gboolean header_serialized;
  gboolean keep_aspect_ratio;
  gint height;
  gint width;
  gsize header_bytes_count;
  guchar header_data[SNIFF_BUFFER_SIZE];
};
...
static gboolean
photos_gegl_buffer_loader_serialize_header (PhotosGeglBufferLoader *self, GError **error)
{
  ...
  if (self->header_serialized)
    goto out;

  istream = g_memory_input_stream_new ();
  for (l = self->header_bytes->head; l != NULL; l = l->next)
    {
      GBytes *bytes = (GBytes *) l->data;
      g_memory_input_stream_add_bytes (G_MEMORY_INPUT_STREAM (istream), bytes);
    }

  ostream = g_memory_output_stream_new (self->header_data, sizeof (self->header_data), NULL, NULL);
  n_spliced = g_output_stream_splice (ostream,
                                      istream,
  ...
  self->header_serialized = TRUE;

 out:
  return self->header_serialized;
}
...
photos_gegl_buffer_loader_find_codec (PhotosGeglBufferLoader *self, GError **error)
{
  ...
  if (!photos_gegl_buffer_loader_serialize_header (self, error))
    goto out;

  if (self->file != NULL)
    path = g_file_get_path (self->file);

  content_type = g_content_type_guess (NULL, self->header_data, self->header_bytes_count, &uncertain);
  if ((uncertain
       || g_strcmp0 (content_type, "application/gzip") == 0
       || g_strcmp0 (content_type, "image/tiff") == 0
       || g_strcmp0 (content_type, "text/plain") == 0)
      && path != NULL)
    {
      g_free (content_type);
      content_type = g_content_type_guess (path, self->header_data, self->header_bytes_count, NULL);
    }
  ...
  if (codec == NULL)
    {
      if (path == NULL)
        {
          g_set_error_literal (error, PHOTOS_ERROR, 0, "Unrecognized image file format");
        }
      else
        {
...
photos_gegl_buffer_loader_load_codec (PhotosGeglBufferLoader *self, const gchar *codec_name, GError **error)
{
  ...
  if (codec_name == NULL)
  ...
  else
    {
      ...
      if (extension == NULL)
        {
          g_set_error (error, PHOTOS_ERROR, 0, "Codec type “%s” is not supported", codec_name);
          goto out;
        }
  ...
  if (self->header_bytes_count > 0)
    {
      if (self->header_serialized)
        {
          g_autoptr (GBytes) bytes = NULL;

          bytes = g_bytes_new_static (self->header_data, self->header_bytes_count);
          if (!photos_gegl_buffer_codec_load_increment (self->codec, bytes, error))
            goto out;
        }
      else
        {
...
static gsize
photos_gegl_buffer_loader_eat_header (PhotosGeglBufferLoader *self, GBytes *bytes, GError **error)
{
  g_autoptr (GBytes) bytes_eaten = NULL;
  gsize count;
  gsize n_bytes_to_eat;
  gsize ret_val = 0;

  g_return_val_if_fail ((gssize) self->header_bytes_count < SNIFF_BUFFER_SIZE, 0);

  count = g_bytes_get_size (bytes);
  n_bytes_to_eat = MIN (SNIFF_BUFFER_SIZE - self->header_bytes_count, count);
  bytes_eaten = g_bytes_new_from_bytes (bytes, 0, n_bytes_to_eat);
  g_queue_push_tail (self->header_bytes, g_bytes_ref (bytes_eaten));

  self->header_bytes_count += n_bytes_to_eat;
  g_return_val_if_fail (self->header_bytes_count <= SNIFF_BUFFER_SIZE, 0);

  if (self->header_bytes_count == SNIFF_BUFFER_SIZE)
    {
...
photos_gegl_buffer_loader_dispose (GObject *object)
{
  ...
  g_queue_free_full (self->header_bytes, (GDestroyNotify) g_bytes_unref);
                                       ...
                                       GError **error)
{
  ...
  if (self->codec == NULL)
    {
      ...
      eaten = photos_gegl_buffer_loader_eat_header (self, bytes, error);
      if (eaten == 0)
        goto out;
  ...
  if (count > 0)
    {
      g_autoptr (GBytes) bytes_remaining = NULL;

      bytes_remaining = g_bytes_new_from_bytes (bytes, offset, count);
      if (!photos_gegl_buffer_codec_load_increment (self->codec, bytes_remaining, error))
        goto out;
    }
