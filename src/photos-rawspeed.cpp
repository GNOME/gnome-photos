                                 ...
                                 guint32 filters)
{
  ...
  while (gegl_buffer_iterator_next (it))
    {
      GeglBufferIteratorItem *item = &it->items[0];
      const GeglRectangle &roi_output = item->roi;
      gfloat *const out = static_cast<gfloat *> (item->data);

      const gint roi_input_height = roi_output.height + 2 * border;
      const gint roi_input_width = roi_output.width + 2 * border;
      const gint roi_input_x = roi_output.x - border;
      const gint roi_input_y = roi_output.y - border;

      GeglRectangle roi_input;
      gegl_rectangle_set (&roi_input,
                          roi_input_x,
                          roi_input_y,
                          static_cast<guint> (roi_input_width),
                          static_cast<guint> (roi_input_height));

      const gint bpp = babl_format_get_bytes_per_pixel (format_input);
      const gint stride = bpp * roi_input.width;
      const gsize input_memory_size = static_cast<gsize> (roi_input.height) * static_cast<gsize> (stride);
      ...
      gegl_buffer_get (buffer_input, &roi_input, 1.0, format_input, chunk_memory, stride, abyss_policy);
      ...
      for (gint j = 0, y = roi_output.y; j < roi_output.height; j++, y++)
        {
          for (gint i = 0, x = roi_output.x; i < roi_output.width; i++, x++)
            {
              if (x >= border && y >= border && x < bbox.width - border && y < bbox.height - border)
                {
                  const gint x_incremented = bbox.width - border;
                  x = x_incremented;
                  i = x_incremented - roi_output.x;

                  if (i >= roi_output.width)
                    break;
                }
                      ...
                      if (y1 >= 0 && x1 >= 0 && y1 < bbox.height && x1 < bbox.width)
                        {
                          ...
                          const gsize offset = static_cast<gsize> (q + border)
                                               * static_cast<gsize> (roi_input.width)
              ...
              for (gsize c = 0; c < 3; c++)
                {
                  const gsize offset_out = 3
                                           * (static_cast<gsize> (j) * static_cast<gsize> (roi_output.width)
                                              + static_cast<gsize> (i))
                                           + c;
          ...
          for (gint i = 0, x = roi_output.x; i < roi_output.width; i++, x++)
            {
              ...
              const gsize offset_in = static_cast<gsize> (j + border) * static_cast<gsize> (roi_input.width)
                                      + static_cast<gsize> (i + border);
              ...
              if (fc == 0 || fc == 2)
                {
                  ...
                  static_assert (border == 3, "Value of border has changed");
                  const gfloat pyb1 = in[offset_in - roi_input.width];
                  const gfloat pyb2 = in[offset_in - 2 * roi_input.width];
                  const gfloat pyb3 = in[offset_in - 3 * roi_input.width];
                  const gfloat pyB1 = in[offset_in + roi_input.width];
                  const gfloat pyB2 = in[offset_in + 2 * roi_input.width];
                  const gfloat pyB3 = in[offset_in + 3 * roi_input.width];
