...
photos_rawspeed_calculate_pattern_color_bayer (const gsize row, const gsize column, const guint32 filters)
{
  ...
  g_return_val_if_fail (0 <= ret_val && ret_val < 4, 0);
                                        ...
                                        guint32 filters)
{
  ...
  const gint n_coefficients = is_4bayer ? 4 : 3;

  for (gint i = 0; i < n_coefficients; i++)
    {
      if (!std::isnormal (image_data->metadata.wbCoeffs[i])
  ...
  return static_cast<GeglBuffer *> (g_object_ref (buffer_output));
}
