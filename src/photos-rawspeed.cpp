...
photos_rawspeed_correct_black_and_white_point (rawspeed::RawImage image, gsize components, guint32 filters)
{
  ...
  GeglBufferIterator *it = gegl_buffer_iterator_new (buffer_output,
                                                     ...
                                                     format_output,
                                                     GEGL_ACCESS_WRITE,
                                                     static_cast<GeglAbyssPolicy> (GEGL_ABYSS_NONE
      ...
      while (gegl_buffer_iterator_next (it))
        {
          ...
          for (gint j = 0, y = item->roi.y; j < item->roi.height && y < bbox_output.height; j++, y++)
            {
              for (gint i = 0, x = item->roi.x; i < item->roi.width && x < bbox_output.width; i++, x++)
                {
                  const gsize in_offset = static_cast<gsize> (crop_y + y) * static_cast<gsize> (width_input)
                                          + static_cast<gsize> (crop_x + x);

                  const gsize out_offset = static_cast<gsize> (j) * static_cast<gsize> (item->roi.width)
                                           + static_cast<gsize> (i);
                  ...
                  for (gsize c = 0; c < components; c++)
                    {
                      const gsize in_offset = components
                                              * (static_cast<gsize> (crop_y + y) * static_cast<gsize> (width_input)
                                                 + static_cast<gsize> (crop_x + x))
                                              + c;

                      const gsize out_offset = components
                                               * (static_cast<gsize> (j) * static_cast<gsize> (item->roi.width)
                                                  + static_cast<gsize> (i))
                                               + c;
