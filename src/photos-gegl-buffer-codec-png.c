...
photos_gegl_buffer_codec_png_info (png_structp png_read_ptr, png_infop png_info_ptr)
{
   ...
   * GEGL, while the width is an unsigned integer (ie. guint or
   * png_uint_32). It must be ensured that the row stride can fit
...
static gboolean
photos_gegl_buffer_codec_png_load_increment (PhotosGeglBufferCodec *codec, GBytes *bytes, GError **error)
{
  PhotosGeglBufferCodecPng *self = PHOTOS_GEGL_BUFFER_CODEC_PNG (codec);
  ...
  png_process_data (self->png_read_ptr, self->png_info_ptr, (guchar *) data, count);
  if (self->fatal_error_occurred)
    {
      ret_val = FALSE;
      goto out;
